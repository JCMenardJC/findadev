...
export class UsersController {
        ...
        async create(@Body() createUserDto: CreateUserDto) {
                ...
                if (verifUser || verifUsername) {
                        return {
                                status: EStatus.ERROR,
                                message: 'Le mail ou le username saisie existent déjà ,veuillez vous connectez, ou bien corriger vos données saisies ',
                        };
                }
        ...
        async findOne(@Request() req) {
                const findId = await this.usersService.findOne(
                        req.user.user_id
                );
                console.log(findId);
        ...
        async remove(@Request() req) {
                ...
                if (!data) {
                        throw new NotFoundException(
                                'Votre compte a déjà était supprimé'
                        );
                }
                const userRemoved = await this.usersService.remove(
                        req.user.user_id
                );
                return {
                        status: EStatus.OK,
